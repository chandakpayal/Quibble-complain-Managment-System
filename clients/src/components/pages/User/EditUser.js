...
const [branch,setBranch] = useState("");
  ...
  useEffect(()=>{
    fetch("http://localhost:9090/api/v1/complainId/"+id).then((res)=>{
      return res.json();
    }).then((resp)=>{
...
const handleSubmit=(e)=>{
      ...
      fetch("http://localhost:9090/api/v1/edit/"+id,{
        method:"PUT",
      ...
      <div className="flex flex-wrap px-4 py-1 w-full">
        <div className="relative w-42 appearance-none label-floating w-full">
          <input
            ...
            type="text"
            placeholder="Enter Status of Branch"
          />
        </div>
      </div>
