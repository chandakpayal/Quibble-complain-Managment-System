...
function User() {
  ...
  useEffect(()=>{
    fetch("http://localhost:9090/api/v1/singleInfo").then((res)=>{
      return res.json();
    }).then((resp)=>{
      setAdmins(resp); 
    }).catch((err)=>{
  ...
  const[InfoUser ,setUserInfo] = useState(null)
    ...
    useEffect(()=>{
        fetch("http://localhost:9090/api/v1/singlecomplain").then((res)=>{
          return res.json();
        }).then((resp)=>{
            setUserInfo(resp); 
      ...
      const RemoveDetails = (_id)=>{
        ...
        if(window.confirm('Do You want to remove ?')){
          fetch("http://localhost:9090/api/v1/delete/"+_id,{
            method:"DELETE"
          }).then((res)=>{
  ...
  return (
      ...
      <div className="sidebar justify-self-start  px-4 w-[300px] h-screen overflow-y-auto text-center shadow-lg shadow-indigo-500/40 bg-white-800">
        <div className="text-white text-xl">
          <div className="p-2.5 mt-1 flex items-center">
            ...
            <h1 className="font-bold text-lg text-white text-[15px] ml-3">
             
            </h1>
          </div>
          <div className="my-2  bg-black-400 h-[10px]" />
         
            <h2 className="text-white bg-purple-500 p-4 rounded-md " > User Dashboard</h2>
          
        </div>

        <button
          className=" w-full p-2.5 mt-8 flex items-center rounded-md px-4 duration-300 cursor-pointer  bg-purple-400 hover:bg-blue-600 text-white"
          onClick={() => {
            setMyDetails(true);
        ...
        <div
          className="p-2.5 mt-3 flex items-center rounded-md px-4 duration-300 cursor-pointer bg-purple-400 hover:bg-red-400 text-white"
          onClick={() => {
            setMyDetails(false);
            setcontractForm(false);
            setMyFile(true)
        ...
        <Link to="/">
          <div className="p-2.5 mt-3 flex items-center rounded-md px-4 duration-300 cursor-pointer bg-purple-400 hover:bg-red-400 text-white">
   ...
   {/* <div  className="bg-cover bg-center h-160 w-full"
          style={{
            backgroundImage:
              "url('https://img.freepik.com/free-photo/3d-rendering-people-avatars-zoom-call_23-2149576736.jpg?w=1060&t=st=1682163241~exp=1682163841~hmac=1df13008701a3ce6602f1990666b735ae035f025bfd0848772cf9c02dc0ae62b')",
          }}>
...
{contractForm && (
      <div className="z-10  bg-purple-600 bg-opacity-50 flex justify-center items-center absolute top-0 right-0 bottom-0 left-0">
      <div className="bg-white py-4 w-4/5  rounded-md text-center">
        ...
        <div className="h-[60vh]  overflow-scroll rounded-lg border border-gray-600 shadow-sm m-5">
          ...
          <tr>
            ...
            <th class="w-59 p-3 text-lg text-bold-500 font-semibold tracking-wide text-left">Branch of Complaint</th>
